...
class LanguageForm extends CFormModel
{
	...
	public function getList_categories(){
		$dbCommand = Yii::app()->db->createCommand("
   			SELECT category FROM `tbl_language` WHERE lang = '$this->lang' GROUP BY `category`
		");
	...
	public function getList_controllers(){
		$dbCommand = Yii::app()->db->createCommand("
   			SELECT controller FROM `tbl_language` WHERE lang = '$this->lang' AND category = '$this->category' AND module = '$this->module' GROUP BY `controller`
		");
		$data = $dbCommand->queryAll();
		$list=array();	 
		foreach ($data as $item){
	...
	public function getList_actions(){
		$dbCommand = Yii::app()->db->createCommand("
   			SELECT action FROM `tbl_language` WHERE lang = '$this->lang' AND category = '$this->category' AND module =  '$this->module' AND controller ='$this->controller' GROUP BY `action`
		");
		$data = $dbCommand->queryAll();
		$list=array();	 
		foreach ($data as $item){
	...
	static function copyLanguage($origin_language,$language){
    	...
    	foreach ($list as $origin_record){
    		...
    		$record->category=$origin_record->category;
