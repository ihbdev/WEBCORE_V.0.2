...
{	
	const EMPLOYEE_GROUP=28;
	const AGENT_GROUP=29;
	const META_LENGTH=25;
	const INTRO_LENGTH=25;
	public function tableName()
	{
		return 'tbl_article';
	}
	/*
	 * Config scope of recruitment
	 */
	public function defaultScope(){
		return array(
			'condition'=>'type = '.Article::ARTICLE_RECRUITMENT,
		);	
	}
	/*
	...
	public $old_title;
	public $old_keyword;
	private $config_other_attributes=array('modified','attach','fulltext','introtext','introimage','list_suggest','metakey','metadesc');	
	...
	public function getUpdate_url()
 	{
 		$url=Yii::app()->createUrl("admin/recruitment/update",array('id'=>$this->id));
		return $url;
 	}
	/*
	 * Get url
	...
	public function getUrl()
 	{
 		$alias=$this->alias;
 		$url=Yii::app()->createUrl("recruitment/view",array('recruitment_alias'=>$alias));
		return $url;
 	}
	/*
	 * Get thumb of video
	...
	public function getThumb_url($type,$class){
		...
		if($this->introimage>0){
			...
			if(isset($image)){
				$src=$image->getThumb('Recruitment',$type);
				if( $image->title != '')	$alt=$image->title;
			}
			else {
				$src=Image::getDefaultThumb('Recruitment', $type);
			}
			return '<img class="'.$class.'" src="'.$src.'" alt="'.$alt.'">';
		}
		else {
			
			return '<img class="img" src="'.Image::getDefaultThumb('Recruitment', $type).'" alt="'.$alt.'">';
		}
	}
	/*
	 * Get image url which view status of recruitment
	 */
	...
	public function getList_similar(){
		if($this->list_suggest != ''){
			...
			foreach ($list as $id){
				$index++;
				if($index <= Setting::s('LIMIT_SIMILAR_RECRUITMENT','Recruitment'))
					$result[]=Recruitment::model()->findByPk($id);
			}
		}
		else {
			...
			$criteria->limit=Setting::s('LIMIT_SIMILAR_RECRUITMENT','Recruitment');
			$result=Recruitment::model()->findAll($criteria);		
		}
		return $result;
	}	
 	/*
	 * Get all specials of class Recruitment
	 * Use in drop select when create, update recruitment
	 */
	...
	return array(
 	 ...
 	 /*
 	 * Get specials of a object recruitment
	...
	public function getImage($type){
		$image=Image::model()->findByPk($this->introimage);
		if(isset($image)){
			$url='<img class="img" src="'.$image->getThumb('Recruitment',$type).'" alt="'.$image->title.'" />';
		}
		else {
			$url='<img class="img" src="'.Image::getDefaultThumb('Recruitment',$type).'" />';
		}
			return $url;	
	}
 	/**
	...
	public function validateUniqueAlias() {
		...
		if ($this->id > 0)
			$criteria->addCondition ( 'id <> ' . $this->id );
		$list = Recruitment::model ()->findAll ( $criteria );
		if (sizeof ( $list ) > 0)
			return false;
		else
			return true;
	...
	public function beforeSave()
	{
		if(parent::beforeSave())
		{
			if($this->isNewRecord)
			{
				...
				$this->status=Recruitment::STATUS_ACTIVE;
				//Set alias
				$alias=iPhoenixString::createAlias($this->title);	
				while(sizeof(Recruitment::model()->findAll('alias = "'.$alias.'"'))>0){
					$suffix=rand(1,99);
					$alias =$alias.'-'.$suffix;
				}
				$this->alias=$alias;			
			...
			$this->type=Article::ARTICLE_RECRUITMENT;
			//Encode fulltext
	...
	{
		$list_recruitment=$this->findAll(array(
		...
		$titles=array();
		foreach($list_recruitment as $recruitment)
			$titles[]=$recruitment->title;
			return $titles;
	}
	/*
	 * Set status of recruitment
	 */
	...
	static function copy($id) {
		...
		if ($command->execute ()) {
			...
			$model = Recruitment::model ()->findByPk ( $copy_id );
			$model->scenario = 'copy';
	...
	static function listAll()
	{
		...
		foreach ($list_category as $id=>$level){					
			...
			$result['cat,'.$id]=array('name'=>$cat->name,'level'=>$level);
			
			//Get child recruitment
			$criteria = new CDbCriteria ();
			$criteria->compare('status', self::STATUS_ACTIVE);
			$criteria->compare('catid',$id);
			$list=Recruitment::model()->findAll($criteria);
			foreach ($list as $recruitment){
				$result['item,'.$recruitment->id]=array('name'=>$recruitment->title,'level'=>$level+1);
			}
		}
		return $result;
	}
