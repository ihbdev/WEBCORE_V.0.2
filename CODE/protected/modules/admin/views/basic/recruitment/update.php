<!--begin inside content-->
	<div class="folder top">
		...
		<div class="folder-header">
			<h1>Quản trị tuyển dụng và đăng kí</h1>
			<div class="header-menu">
				<ul>
					<li><a class="header-menu-active new-icon" href=""><span>Chỉnh sửa tuyển dụng<?php echo $model->title;?></span></a></li>					
				</ul>
			</div>
		</div>
		<!--end title-->
		...
		<div>
            	<input type="button" class="button" value="Thêm tuyển dụng" style="width:180px;" onClick="parent.location='<?php echo Yii::app()->createUrl('admin/recruitment/create')?>'"/>
                <input type="button" class="button" value="Danh sách tuyển dụng" style="width:180px;" onClick="parent.location='<?php echo Yii::app()->createUrl('admin/recruitment')?>'"/>
                <div class="line top bottom"></div>	
            </div>
		<?php $form=$this->beginWidget('CActiveForm', array('method'=>'post','enableAjaxValidation'=>true,'htmlOptions'=>array('enctype' => 'multipart/form-data'))); ?>	
			<!--begin left content-->
			<div class="fl">
				<ul>
					<div id="above_row">
					<div id="left_row">
						...
						<div class="row">
							<li>
								...
								<?php  
									echo CHtml::link("Download",array('/site/download','path'=>$model->attach),array('target'=>'_blank'));
								?>
							</li>
						</div>
						<?php endif;?>
							<div class="row">
							<li>
								<label>Upload file đính kèm mới</label>
								<?php echo $form->fileField($model, 'attach');?>
					...
					<div id="right_row">	
                    	...
                    	<div class="row">
							<li>
								...
								<!-- <input type="button" id="btn-add-product" class="button" value="Chọn trang tĩnh" style="width:60px;padding:1px;margin-top:-5px;" onclick="showPopUp();"/>-->
...
<ul>
	<?php 
		Yii::app()->clientScript->registerScript('search-recruitment-suggest', "
			$('#recruitment-search').submit(function(){
			$.fn.yiiGridView.update('recruitment-list', {
				data: $(this).serialize()});
				return false;
			});");
	?>
	 <?php $form=$this->beginWidget('CActiveForm', array('method'=>'get','id'=>'recruitment-search')); ?>
	 <li>
     	<?php echo $form->labelEx($model,'title'); ?>
       	<?php $this->widget('CAutoComplete', array(
        	            	'model'=>$suggest,
                         	'attribute'=>'title',
							'url'=>array('recruitment/suggestTitle'),
							'htmlOptions'=>array(
								'style'=>'width:230px;',
								),
						)); ?>								
	...
	<li>
		...
		<?php echo $form->dropDownList($suggest,'catid',$list,array('style'=>'width:200px','name'=>'SuggestRecruitment[catid]')); ?>
	</li>            
	<li>
	<label>&nbsp;</label> 
	<input type="submit" class="button" value="Lọc trang tĩnh">
	</li>
	<?php $this->endWidget(); ?>	
	<li>
	  <?php 
			$this->widget('iPhoenixGridView', array(
  				...
  				'summaryText'=>'Có {count} trang tĩnh',
  		...
  		function(){  	
			...
			jQuery.ajax({
				...
				success:function(data){
					$('#Recruitment_list_suggest').val(data);
					hidenPopUp();
				},
				type:'POST',
				url:'<?php echo $this->createUrl('recruitment/updateSuggest');?>',
				'cache':'false'});
			return false;
		});
</script>